...
function Login({ onLogin, onRegister }) {
  ...
  } catch (err) { setError(err.data?.errors?.credentials || err.data?.errors?.email || err.data?.errors?.phone || err.message || 'Sign in failed. Check your details.'); } finally { setBusy(false); } };
...
function Register({ onNavigate }) {
  ...
  const update = (field) => (event) => setForm({ ...form, [field]: event.target.value });
    const submit = async (event) => { event.preventDefault(); setBusy(true); setError(''); setSuccess(''); try { const result = await apiClient.post('/auth/register', { ...form, verificationMethod: form.verificationMethod }); savePendingRegistration({ ...result.data, phone: form.phone }); onNavigate('/verify-otp'); } catch (err) { sessionStorage.removeItem(PENDING_REGISTRATION_KEY); const fieldErrors = err.data?.errors ? Object.values(err.data.errors).filter(Boolean).join(' ') : ''; const message = err.message === 'REDIS_URL is required for registration challenges' ? 'Registration is temporarily unavailable. Please try again later.' : err.message; setError(fieldErrors || message || 'Registration failed. No user account has been created yet.'); } finally { setBusy(false); } };
...
function OtpInput({ value, onChange }) {
  ...
  const updateDigit = (index, nextValue) => {
    ...
    const nextDigits = [...digits]; nextDigits[index] = digit; onChange(nextDigits.join(''));
  ...
  const handlePaste = (event) => { event.preventDefault(); const pasted = event.clipboardData.getData('text').replace(/\D/g, '').slice(0, 6); onChange(pasted); inputRefs.current[Math.min(pasted.length, 5)]?.focus(); };
  return <div className="otp-inputs" role="group" aria-label="Six-digit verification code">{digits.map((digit, index) => <input key={index} ref={(element) => { inputRefs.current[index] = element; }} className="otp-digit" inputMode="numeric" pattern="[0-9]" maxLength="1" value={digit} onChange={(event) => updateDigit(index, event.target.value)} onKeyDown={(event) => handleKeyDown(index, event)} onPaste={handlePaste} aria-label={`Verification digit ${index + 1}`} required />)}</div>;
}
...
function Farms({ farms, onCreated, onUpdated, onDeleted }) {
  ...
  const create = async (event) => { event.preventDefault(); setBusy(true); setError(''); try { const result = await apiClient.post('/farms', { name, region: location }); onCreated(result.data); setName(''); setLocation(''); setOpen(false); } catch (err) { setError(err.message || 'Could not create farm.'); } finally { setBusy(false); } };
...
function CropManagement({ farms, onBack }) {
  ...
  const openCycleDetails = async (cycle) => {
    try {
      const result = await apiClient.get(`/farms/${farmId}/crops/${cycle.id}`);
      setSelectedCycle(result.data || cycle);
    } catch (err) {
      setError(err.message || 'Unable to load crop production details.');
  ...
  return <section className="record-page">
    ...
    {!farms.length ? <div className="panel empty-wide"><h3>Create a farm first</h3></div> : <>
      ...
      {loading ? <div className="loading-line" aria-label="Loading crop records" /> : <div className="farm-grid">{cycles.map((cycle) => <article className="farm-card" key={cycle.id}><div className="farm-card-top"><span className="farm-avatar large">{(cycle.crop?.name || 'C')[0]}</span><span className="live-badge">{cycle.status}</span></div><h3>{cycle.cycleName || cycle.crop?.name || 'Crop cycle'}</h3><p>{cycle.crop?.name || 'Crop'}{cycle.variety?.name ? ` · ${cycle.variety.name}` : ''}</p><p>{cycle.plantedArea} {cycle.areaUnit} · {cycle.field?.name || 'Field not set'}</p><div className="farm-card-foot"><button className="text-button" type="button" onClick={() => openCycleDetails(cycle)}>Details</button><button className="text-button" type="button" onClick={() => edit(cycle)}>Edit</button><button className="text-button" type="button" onClick={() => remove(cycle)} disabled={busy}>Delete</button></div></article>)}{!cycles.length && <div className="panel empty-wide"><h3>No crop cycles yet</h3><p className="muted">Add a production record after selecting a field and crop type.</p></div>}</div>}
      {selectedCycle && !showForm && <div className="record-content-panel crop-detail"><div><p className="eyebrow">CROP DETAIL PROFILE</p><h3>{selectedCycle.cycleName || selectedCycle.crop?.name}</h3><p className="muted">{selectedCycle.crop?.name} · {selectedCycle.variety?.name || 'Variety not recorded'}</p></div><div className="activity-detail-grid"><p><strong>Field</strong><span>{selectedCycle.field?.name || 'Not set'}</span></p><p><strong>Planting</strong><span>{selectedCycle.plantingDate?.slice(0, 10) || 'Not recorded'}</span></p><p><strong>Expected harvest</strong><span>{selectedCycle.expectedHarvestDate?.slice(0, 10) || 'Not set'}</span></p><p><strong>Area</strong><span>{selectedCycle.plantedArea} {selectedCycle.areaUnit}</span></p><p><strong>Status</strong><span>{selectedCycle.status}</span></p><p><strong>Yield target</strong><span>{selectedCycle.summary?.expectedYield || selectedCycle.expectedYield || 0} {selectedCycle.summary?.yieldUnit || selectedCycle.yieldUnit || 'KG'}</span></p><p><strong>Harvested</strong><span>{selectedCycle.summary?.totalHarvest || selectedCycle.actualYield || 0} {selectedCycle.summary?.yieldUnit || selectedCycle.yieldUnit || 'KG'}</span></p><p><strong>Profit</strong><span>GHS {Number(selectedCycle.summary?.netProfit ?? 0).toLocaleString()}</span></p></div>{selectedCycle.summary && <div className="activity-detail"><p className="eyebrow">PRODUCTION SUMMARY</p><div className="stat-grid records-summary"><Stat label="Activities" value={selectedCycle.summary.totalActivities || 0} detail="Recorded tasks" tone="blue" /><Stat label="Inputs" value={selectedCycle.summary.totalInputs || 0} detail="Material records" tone="yellow" /><Stat label="Observations" value={selectedCycle.summary.totalObservations || 0} detail="Field checks" tone="green" /><Stat label="Revenue" value={`GHS ${Number(selectedCycle.summary.totalRevenue || 0).toLocaleString()}`} detail="Cash inflow" tone="green" /><Stat label="Expenses" value={`GHS ${Number(selectedCycle.summary.totalExpenses || 0).toLocaleString()}`} detail="Production costs" tone="red" /><Stat label="Net result" value={`GHS ${Number(selectedCycle.summary.netProfit || 0).toLocaleString()}`} detail="Profit / loss" tone="blue" /></div>{selectedCycle.summary.timeline?.length > 0 && <div className="list-stack">{selectedCycle.summary.timeline.map((entry, index) => <div key={`${entry.label}-${entry.date}-${index}`} className="farm-row"><span className="farm-avatar large">{entry.label.slice(0,1)}</span><div><strong>{entry.label}</strong><small>{new Date(entry.date).toLocaleDateString()} · {entry.description}</small></div><span className="row-arrow">→</span></div>)}</div>}</div>}</div>}
    </>}
  </section>;
}
...
function CommunityFeed({ user }) {
  ...
  const react = async (post, liked) => {
    try {
      ...
      const liker = { id: user?.id, firstName: user?.firstName || '', lastName: user?.lastName || '' };
      setPosts(posts.map((item) => item.id === post.id ? { ...item, _count: { ...item._count, likes: item._count.likes + (liked ? -1 : 1) }, likers: liked ? (item.likers || []).filter((entry) => entry.id !== user?.id) : [...(item.likers || []), liker], viewerLiked: !liked } : item));
    } catch (err) {
...
function CommunityPost({ post, onLike, currentUserId, onEdit, onDelete, onSaveEdit, editingPostId, editingBody, setEditingBody, editingCategory, setEditingCategory }) {
  ...
  const [comments, setComments] = useState(post.comments || []); const [commentBody, setCommentBody] = useState(''); const [commentsOpen, setCommentsOpen] = useState(true); const [commentBusy, setCommentBusy] = useState(false);
    const toggleComments = async () => { if (!commentsOpen) { try { const result = await apiClient.get(`/community/posts/${post.id}/comments`); setComments(result.data || []); } catch (err) { console.error(err); } } setCommentsOpen(!commentsOpen); };
  ...
  return (
    <article className="community-post">
      ...
      <div className="community-actions">
        ...
        <span className="liker-summary">{post.likers?.length ? `Liked by ${post.likers.slice(0, 3).map((liker) => `${liker.firstName} ${liker.lastName}`.trim()).join(', ')}${post.likers.length > 3 ? ` and ${post.likers.length - 3} more` : ''}` : 'No likes yet'}</span>
      </div>
      {commentsOpen && <div className="comments-panel"><div className="comment-list">{comments.map((comment) => <div className="comment" key={comment.id}><strong>{comment.author?.firstName} {comment.author?.lastName}</strong><p>{comment.body}</p><time dateTime={comment.createdAt}>{new Date(comment.createdAt).toLocaleString()}</time></div>)}{!comments.length && <small className="muted">No comments yet.</small>}</div><form className="comment-form" onSubmit={submitComment}><input value={commentBody} onChange={(event) => setCommentBody(event.target.value)} placeholder="Write a comment" maxLength="2000" required /><button className="primary-button" disabled={commentBusy}>{commentBusy ? 'Posting...' : 'Comment'}</button></form></div>}
    </article>
  );
}
...
function Account({ user, onUpdated, onPasswordChanged }) {
  ...
  const [passwordBusy, setPasswordBusy] = useState(false); const [passwordError, setPasswordError] = useState(''); const [passwordSuccess, setPasswordSuccess] = useState('');
  ...
  const changePassword = async (event) => { event.preventDefault(); setPasswordBusy(true); setPasswordError(''); setPasswordSuccess(''); try { const result = await apiClient.post('/auth/change-password', passwordForm); setPasswordForm({ currentPassword: '', newPassword: '', confirmPassword: '' }); setPasswordSuccess(result.message || 'Password changed successfully. Please log in again.'); window.setTimeout(onPasswordChanged, 900); } catch (err) { const validationErrors = err.data?.errors || {}; setPasswordError(validationErrors.password || validationErrors.newPassword || validationErrors.confirmPassword || validationErrors.currentPassword || err.message || 'Could not change your password.'); } finally { setPasswordBusy(false); } };
  const update = (field) => (event) => setForm({ ...form, [field]: event.target.value });
  const updatePassword = (field) => (event) => setPasswordForm({ ...passwordForm, [field]: event.target.value });
  ...
  return <section><div className="section-heading"><div><p className="eyebrow">ACCOUNT</p><h2>Personal details</h2><p className="muted">Keep your FarmWise contact details current.</p></div></div><div className="account-sections"><form className="account-form" onSubmit={submit}>{error && <div className="notice error">{error}</div>}<div className="profile-picture-row"><div className="profile-picture">{form.profilePictureUrl ? <img src={form.profilePictureUrl} alt="Profile" /> : (user?.firstName || 'F')[0].toUpperCase()}</div><label className="picture-picker">Profile picture<input type="file" accept="image/png,image/jpeg,image/webp" onChange={selectPicture} /></label></div><div className="form-row"><label>First name<input value={form.firstName} onChange={update('firstName')} required autoComplete="given-name" /></label><label>Last name<input value={form.lastName} onChange={update('lastName')} required autoComplete="family-name" /></label></div><label>Phone number<input value={form.phone} onChange={update('phone')} required autoComplete="tel" /></label><label>Email address<input value={user?.email || ''} disabled /></label><button className="primary-button" disabled={busy}>{busy ? 'Saving...' : 'Save changes'} <span>→</span></button></form><form className="account-form" onSubmit={changePassword}><div><p className="eyebrow">SECURITY</p><h3>Change password</h3><p className="muted">Use a new password to keep your account secure.</p></div>{passwordError && <div className="notice error" role="alert">{passwordError}</div>}{passwordSuccess && <div className="notice success" role="status">{passwordSuccess}</div>}<label>Current password<PasswordInput value={passwordForm.currentPassword} onChange={updatePassword('currentPassword')} autoComplete="current-password" /></label><label>New password<PasswordInput value={passwordForm.newPassword} onChange={updatePassword('newPassword')} autoComplete="new-password" /></label><label>Confirm new password<PasswordInput value={passwordForm.confirmPassword} onChange={updatePassword('confirmPassword')} autoComplete="new-password" /></label><button className="primary-button" disabled={passwordBusy || Boolean(passwordSuccess)}>{passwordBusy ? 'Changing...' : 'Change password'} <span>→</span></button></form></div></section>;
}
