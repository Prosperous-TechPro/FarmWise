...
function Account({ user, onUpdated, onPasswordChanged }) {
  ...
  const changePassword = async (event) => { event.preventDefault(); setPasswordBusy(true); setPasswordError(''); setPasswordSuccess(''); try { const result = await apiClient.post('/auth/change-password', passwordForm); setPasswordForm({ currentPassword: '', newPassword: '', confirmPassword: '' }); setPasswordSuccess(result.message || 'Password changed successfully. Please log in again.'); window.setTimeout(onPasswordChanged, 900); } catch (err) { setPasswordError(err.data?.errors?.password || err.message || 'Could not change your password.'); } finally { setPasswordBusy(false); } };
  const update = (field) => (event) => setForm({ ...form, [field]: event.target.value });
